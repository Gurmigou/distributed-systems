...
public class Main {
    ...
    private static void initThreads() {
        for (int i = 0; i < threads.length; i++) {
            if (i == 0)
                threads[i] = new Thread(new Recruits(recruits, barrier, i, 0, (i + 1) * (SIZE / PARTS_COUNT) + 1));
            else if (i == threads.length - 1)
                threads[i] = new Thread(new Recruits(recruits, barrier, i, (i) * (SIZE / PARTS_COUNT) - 1, (i + 1) * (SIZE / PARTS_COUNT)));
            else
                threads[i] = new Thread(new Recruits(recruits, barrier, i, (i) * (SIZE / PARTS_COUNT) - 1, (i + 1) * (SIZE / PARTS_COUNT) + 1));
        }
